...
old_smtp_ip_1=$(aws ec2 describe-instances --instance-ids $OLD_SMPT_1 \
...
aws elb deregister-instances-from-load-balancer --load-balancer-name $smtp_loadbalancer --instances $OLD_SMPT_1


#ssh to old smtp and check mail qu before removing the ENI
ssh -oStrictHostKeyChecking=no ec2-user@$old_smtp_ip_1 "./checkmailq.sh"
